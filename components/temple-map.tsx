...
export default function TempleMap() {
  ...
  const handleStateChange = (value) => {
    if (value === "all") {
      ...
      const templesInState = temples.filter((t) => t.state === value)
      if (templesInState.length > 0) {
        const avgLat = templesInState.reduce((sum, t) => sum + t.lat, 0) / templesInState.length
        const avgLng = templesInState.reduce((sum, t) => sum + t.lng, 0) / templesInState.length
        setMapCenter([avgLat, avgLng])
        setMapZoom(6) // closer to state level
      }
    }
  }
